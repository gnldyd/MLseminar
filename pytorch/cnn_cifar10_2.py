...
def get_mean_std(data_path):
    ...
    return train_dataset.data.mean(axis=(0, 1, 2)) / 255, train_dataset.data.std(axis=(0, 1, 2)) / 255
...
def run(parallel_train=False, gpu_name="cuda", seed_value=1216, data_path="./", batch_size=100, shuffle=False, learning_rate=0.001, training_epochs=100, printable=True, load_model=False, load_model_path="./model.pt", save_model=False, save_model_path="./model.pt"):
    ...
    transform = transforms.Compose([
        ...
        transforms.Normalize((mean), (std))  # MNIST 데이터의 (평균 데이터 값, 표준 편차 값) 설정
    ])

    # DataLoader 생성
    train_loader, test_loader = get_loaders(data_path, transform, batch_size, shuffle)

    model = models.AlexNet(10)  # 모델 생성
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)  # 최적화 설정
...
if __name__ == "__main__":
    ...
    # 기존 모델 사용
    load_model = False
    load_model_path = model_path + "model_AlexNet_cifar10.pt"

    # 학습한 모델 저장
    save_model = True
    save_model_path = model_path + "model_AlexNet_cifar10.pt"
